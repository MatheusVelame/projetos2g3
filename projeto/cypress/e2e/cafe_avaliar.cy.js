describe('test suit avaliar cafe', () => {
  ...
  it('cenario3', () => { // Vou deixar uma avaliação, logado com o e-mail “mauriciodesouza@gmail.com” e senha “mauricio123”,  na Cafeteria da Esquina e ela não aparecerá na Cafeteria Panela da Roça, e não conseguirei.
    ...
    cy.visit('/')
    cy.wait(2000);
    cy.contains('.card-title', 'Cafeteria Panela da Roca').parent().within(() => {
      // Dentro dessa carta, encontre e clique no botão "Ver mais"
      cy.get('a[href*="/detalhes/"]').contains('Ver mais').click();
    });
    cy.wait(3000);
    ...
    cy.visit('/')
    cy.wait(2000);
    cy.contains('.card-title', 'Cafeteria da Esquina').parent().within(() => {
      // Dentro dessa carta, encontre e clique no botão "Ver mais"
      cy.get('a[href*="/detalhes/"]').contains('Ver mais').click();
    });
    cy.wait(3000);
